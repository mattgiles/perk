...
`review-angle-selector`, `draft-reviewer`, `harvest-analyst`). An absent key uses
...
automatically at `/ready`. Checks run concurrently (report in declared order), so each row must be
