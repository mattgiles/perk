...
> Note: the workflow-level `model` default the wave module passes — not a settings override
> map — is the configuration mechanism: it is applied at spawn time and wins over the agent
> def's model.
