...
2. **Explore read-only.** Plan mode is read-only on purpose: ground the design in real files and
   ...
   pass over the claims the design prose makes — delegate it to `run_scout_wave` (independent,
   self-contained read-only briefs on fresh `perk.scout` lanes; its guidelines carry the
   mechanics) and explore small questions directly. Never delegate the goal framing, the roadmap
   decomposition, or the user conversation, and let no design decision rest on a scout claim
   whose `pointer` you have not re-read yourself.
