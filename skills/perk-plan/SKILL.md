...
answered from the checkout alone; a question that needs your running context, or whose answer
decides the next question, is yours to explore directly. Before the plan relies on a scout
finding, re-read its `pointer` yourself, and record in `## Assumptions` which claims you verified
and which briefs failed or went unanswered. The plan's judgment and authoring stay with you.
