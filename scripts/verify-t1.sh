#!/usr/bin/env bash
# Phase 0 · Turn 1 — hard-gate verification.
# Runs the five hard-gate checks from docs/phase-0-turn-1.md §9.
# Assumes: run from repo root; `perk` installed (editable) and on PATH; `pi`, `uv`,
# `npm`, `python3` available.
set -uo pipefail
cd "$(dirname "$0")/.."
ROOT="$(pwd)"
fail=0
...
# Watchdog runner (macOS has no `timeout`).
run_to() {
  local secs=$1; shift
  "$@" & local pid=$!
...
PERK_V="$(perk --version 2>/dev/null | awk '{print $2}')"
PKG_V="$(node -e 'process.stdout.write(require("./package.json").version)' 2>/dev/null)"
PY_V="$(python3 -c 'import perk; print(perk.__version__)' 2>/dev/null)"
...
rm -rf dist *.tgz
# Membership is tested via Python's zipfile/tarfile (atomic central-directory read,
# no unzip/grep/pipe nondeterminism). uv build / npm pack self-terminate.
uv build >/tmp/perk-build.log 2>&1
WHL="$(ls dist/*.whl 2>/dev/null | head -1)"
if [ -n "$WHL" ] && python3 -c "import sys,zipfile; sys.exit(0 if 'perk/_shared/README.md' in zipfile.ZipFile(sys.argv[1]).namelist() else 1)" "$WHL"; then
...
if [ -n "$TGZ" ] && python3 -c "import sys,tarfile; n=set(tarfile.open(sys.argv[1]).getnames()); sys.exit(0 if {'package/shared/README.md','package/extension/index.ts'} <= n else 1)" "$TGZ"; then
