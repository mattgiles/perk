...
- `perk-dev audit attribution <jsonl>...` — the transcript-composition attribution over explicit session JSONL files: whole-file raw-line char accounting (per-kind and per-tool rows, lexical read-path classes, top-10 largest results as provenance-only rows, header/malformed/off-branch reconciliation), built on the session read edge's new additive `raw_chars`/`malformed_chars` metric and the promoted `select_active_branch`/`pair_executions` seams; report-only and dev-only, with the fresh baseline recorded in `docs/design/context-payload-baseline-2.md` (b3a19b54)
