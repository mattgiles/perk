...
- `perk learn harvest` — the objective-factory cold door that mines `docs/learned/` as lenses into the code: it fast-forwards the invocation checkout when it cleanly can (a guarded, best-effort sync), gathers the selected docs into a run-scoped versioned manifest (`commit_sha` = HEAD at gather time), and launches a read-only objective-authoring session that grounds every mined opportunity and curates ONE bounded improvement objective (single lane in phase 1 — a larger selection is refused `selection_too_large`; ≤ 8 roadmap nodes, backlog-with-reasons, honest zero-opportunity stop) (04efb277)
