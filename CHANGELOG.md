...
- `perk learn harvest` — the objective-factory cold door that mines `docs/learned/` as lenses into the code: it fast-forwards the invocation checkout, gathers the selected docs into a run-scoped versioned manifest (`commit_sha` = the gather revision), and launches a read-only objective-authoring session that grounds every mined opportunity and curates ONE bounded improvement objective (single lane in phase 1 — a larger selection is refused `selection_too_large`; ≤ 8 roadmap nodes, backlog-with-reasons, honest zero-opportunity stop) (04efb277)
