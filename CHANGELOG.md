...
- `run_ci` now streams a live one-line progress indicator while checks run — per-check `✓`/`✗`/`⊘`/`…` glyphs in declared order plus a ticking elapsed-seconds counter, replaced in place on the tool row via pi's partial-result channel (UI-only; the deterministic final report is unchanged and supersedes it) (e8897bdc)
