...
- **pi-extension** — Pi SDK/extension substrate craft — API facts, context injection/loading, seams, TUI surfaces, tool-param decode, headless session driving. (pi/context-injection, pi/context-system, pi/extension-api, pi/extension-seams, pi/headless-session-drive, pi/native-sdk-bridge, pi/tool-param-decode, pi/tui-surfaces)
...
- **toolchain-gotchas** — Lint/typecheck/test toolchain gotchas — Biome/tsc, ruff, ty, node:test determinism, test parallelism, worktree node_modules, Astro/Starlight docs-site. (toolchain/biome, toolchain/docs-site-astro-starlight, toolchain/jsdom-react-component-harness, toolchain/node-test-async-determinism, toolchain/pytest-prompt-hermeticity, toolchain/ruff, toolchain/test-parallelism, toolchain/ty, toolchain/worktree-node-modules)
...
- **doors-and-launch** — CLI↔session plumbing — cold-door launch/client, warm-door commands, CLI command groups, factory/seeded doors, write-capable doors, the remote runner. (workflow/cli-command-groups, workflow/cli-startup-tiers, workflow/cold-door-client, workflow/cold-door-launch, workflow/plan-factories, workflow/remote-runner, workflow/startup-profiling, workflow/warm-door-commands, workflow/write-capable-cold-doors)
- **plan-lifecycle** — The plan artifact's life — plan-ref linkage, review→approval→save, worktrees, run identity, mergeability + conflict resolution, git substrate, plan currency. (workflow/git-substrate, workflow/mergeability-and-conflict-resolution, workflow/plan-ref-lifecycle, workflow/plan-review-flow, workflow/plan-save-surfaces, workflow/planning-time-facts-decay, workflow/session-data, workflow/worktree-lifecycle)
...
- **knowledge-stewardship** — Keeping the record true — the /learn evidence pipeline and harvest core, doc-reconciliation craft, session-audit expectation curation, binding design records. (workflow/binding-design-records, workflow/curation-batch-craft, workflow/doc-reconciliation, workflow/learn-docs-scan, workflow/learn-evidence-pipeline, workflow/learn-harvest-dream-core, workflow/session-audit-expectations)
- **quality-and-guards** — Code-quality disciplines — source-scan guard tests, test-pin sweeps, broad-catch narrowing, Pydantic boundary models, lease-fenced outbox delivery. (workflow/broad-catch-narrowing, workflow/lease-outbox-delivery, workflow/pydantic-boundary-models, workflow/source-scan-guards, workflow/test-pin-sweeps, workflow/vacuity-proof-tests)
- **prose-governance** — Model-facing prose governance — the prose-map catalog/scenarios, the perk-dev prose-review workbench (FastAPI/Vite), source adapters, the edit workspace. (workflow/prose-review-workbench)
<!-- END perk docs-sync -->
