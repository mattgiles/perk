...
- **workflow/learn-harvest** — You are touching src/perk/learn/harvest.py, a docs-harvest consumer or lane ceiling, a path-selection containment API, or testing a pipeline-fed ordering contract.
...
- **workflow/objective-delivery** — You are touching src/perk/delivery/ (journal, TrainPersistence, train/stack probes, stacked /submit publication), delivery_order, a delivery/recovery node, or stacked-delivery headers.
...
- **workflow/session-audit-expectations** — You are curating perk-dev audit expectations (expectations.yaml), the audit census/vintage reckoning, writing a session-audit checker, bounding evidence packets, or citing the read-only gate.
