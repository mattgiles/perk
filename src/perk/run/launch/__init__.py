...
→ setup → exec is unchanged) and the ``--dry-run`` preview (:func:`_emit_dry_run_preview`).
**The exec seam lives in :mod:`perk.run.pi_exec`** — the one Pi executor :func:`exec_pi`, the
agent-dir resolution (:func:`resolve_launch_agent_dir` → :class:`LaunchAgentDir`), the pure
child-environment builder, the agent-lock sweep and the maintainer-only stop-before-exec arm; it
sits below the CLI on the bare-``perk`` import tier, so the plain session never imports this
package. This facade re-exports only the three names the orchestrator itself uses (``exec_pi``,
``LaunchAgentDir``, ``resolve_launch_agent_dir``) — :func:`_exec_pi` adapts a
:class:`_LaunchContext` onto ``exec_pi`` read as a facade global, so ``setattr(launch, "exec_pi",
…)`` still rebinds what the stage launch calls; the private exec seams are deliberately NOT
re-exported (a ``launch.<private>`` patch would be a silent no-op against the pipeline — patch
``pi_exec.<name>`` / ``pi_exec.os``). The module-level imports the string-path monkeypatches
resolve against (``os`` / ``subprocess`` / ``github`` / ``git`` / ``cache`` / ``linear_agent`` /
``init`` / ``runner``) are kept here so ``perk.run.launch.<mod>.attr`` rebinds the shared
singleton every submodule that imports the same module sees. The orchestrator and its phase
functions reference the moved helpers (``resolve_target`` / ``resolve_worktree`` /
``_resolve_prompt`` / ``fetch_plan_body`` / ``write_plan_body_snapshot`` / ``materialize_skills``
/ ``materialize_extensions`` / ``print_launch_banner`` / ``run_worktree_setup`` /
``_drive_remote_target`` / ``exec_pi``) as **bare facade globals**, so ``setattr(launch,
"run_worktree_setup", …)`` / ``setattr(launch, "launch_stage", …)`` keep rebinding the names the
phases read (zero test churn) — which is also why the phases live HERE, not in a ``phases``
submodule: a submodule would freeze its own copies of those names and silently break the
name-binding patches. ``_WORKTREE_SETUP_TIMEOUT_S`` travels with ``run_worktree_setup`` into
...
from perk.run.pi_exec import LaunchAgentDir, exec_pi, resolve_launch_agent_dir
