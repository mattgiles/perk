...
``pi_cli_missing``, ``launch_failed``, selection errors) · 2 not-a-repo · a successful exec
never returns (the terminal receives pi's own exit status).
"""
...
def resume_session(
    ...
    """Browse and reopen Pi conversations for a checkout (pi --resume).
    ...
    plan's PR: its number or pasted .../pull/N URL. Run ids are not yet accepted. Checkouts
    are never created, restored, or rebound (perk implement does that); a missing or
    mismatched checkout is a typed refusal.
    ...
    try:
        ...
        if target is not None:
            ...
            # needs a working `gh`; a Linear-backed repo authenticates through its own errors.
        ...
        if dry_run:  # side-effect-free: no selector, no handoff, no run id
            ...
            return
        user_output(f"opening Pi's session picker in {checkout}: {shlex.join(spec.argv)}")
