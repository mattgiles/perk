"""``perk workflow run`` — the supervisor read surface over dispatched runs.
...
dispatch records (the verified ``run_id → plan`` linkage) and correlates each
...
``cancel``/``retry`` are the shipped control siblings of ``list`` (contracts.md §8.18):
