...
commands stay flat single files (`{name}_cmd.py`). Root registration is
`cli.py::_register_root_commands`, deferred to Click's first subcommand lookup (§8.3) — this
package is never imported by bare `perk` or `perk --version`.
"""
