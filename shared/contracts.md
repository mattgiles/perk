...
id only) and a real run uses `select_plan(main_repo_root(...))` with
its one canonical read (PR selectors included); the no-argument form reads the invoking checkout's `cache.plan-ref` and
