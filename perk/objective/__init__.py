...
- ``_models`` — the type leaf: module constants/markers, :class:`NodeStatus` + its category sets,
  the dataclasses (:class:`ObjectiveNode` / :class:`ObjectiveHeader` / :class:`PlanSelection` /
  :class:`DependencyGraph`), and the dual-encoding marker helpers.
- ``parse`` — the roadmap-block readers/validators.
...
- ``graph`` — the phase/sort helpers, node mutation, node↔PR matchers, and graph constructors.
"""
