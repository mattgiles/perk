...
for (const mode of ["pr-rebase", "retained-continuation"] as const) {
  const schema = conflictResolutionSchema(mode);
  const completed = mode === "pr-rebase" ? completedResolution : completedRetainedResolution;
  test(`installed public foreground compatibility (offline): ${mode}`, {
    skip:
    ...
    const cwd = join(scratch, `sync-${RETAINED_OPERATION}`);
    execFileSync("git", ["-C", parent, "worktree", "add", "--detach", cwd], { timeout: 30_000 });
    mkdirSync(join(cwd, ".pi/agents/perk"), { recursive: true });
    const task =
      mode === "pr-rebase"
        ? conflictResolutionTask(cwd)
        : retainedConflictResolutionTask(retainedDispatch(cwd));
    assert.ok(task);
    writeFileSync(
    ...
    const forceAsync = (await jiti.import(
      join(root, "src/runs/background/top-level-async.ts"),
    )) as {
      ...
      async () => {
        ...
        const b = bridge.registerPromptTemplateDelegationBridge({
          ...
          async execute(_id, params, _signal, _ctx, update) {
            ...
            assert.equal(params.task, task);
            if (mode === "retained-continuation") {
              assert.ok(
                String(params.task).includes(
                  `\nRETAINED-CONTINUATION SENTINEL: resume the in-progress rebase in ${cwd}\n`,
                ),
              );
              assert.match(String(params.task), /node 2.1, branch plan-91, PR #91/);
              assert.match(String(params.task), /Never start a rebase, push, abort/);
              assert.doesNotMatch(String(params.task), /force-with-lease/);
            } else assert.doesNotMatch(String(params.task), /RETAINED-CONTINUATION SENTINEL/);
        ...
        try {
          const result = await e.resolve(request(cwd));
          assert.equal(
            result.kind,
            mode === "pr-rebase" ? "resolved" : "continuation-ready",
      ...
      await t.test(
        ...
        async () => {
          ...
          const tuple = {
            ...
            ownerRunId: "parent",
            nodeId: mode === "pr-rebase" ? "submit-conflict" : "retained-conflict",
          };
    ...
    await t.test(
      ...
      async () => {
        ...
        for (const [i, variant] of variants.entries()) {
          const worktree = join(scratch, `profile-${i}`, `sync-${RETAINED_OPERATION}`);
          mkdirSync(join(worktree, ".pi/agents/perk"), { recursive: true });
          execFileSync("git", ["init", "-q", worktree], { timeout: 30_000 });
          writeFileSync(
          ...
          const r = await e.resolve(request(worktree));
          assert.ok(
