...
const TOOL_GUIDELINES = [
  ...
  "A held result before browser readiness is flushed by the readiness notice (findings: [] is the pure retry — never a timer); a held result after readiness means the server stayed unreachable through the tool's bounded retries — present those findings in-session, no later wake is promised (that in-session presentation IS the degrade for this flow). A held result before readiness is never a degrade — the door reports browser readiness itself.",
