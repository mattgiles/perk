...
// rule-specific bodies. The policy-free graph machinery (lexer, edge map, cycle detector, and the
// generic direction/census/token helpers) lives in the dev-only `extension/testing/importGraph.ts`
// (with its own non-vacuity controls in `extension/testing/importGraph.test.ts`); the exact-pinned
// `typescript` devDependency import moved with it, so production sources still gain no imports
// (`bareImportGuard.test.ts` scans production files only and package.json runtime `dependencies`
// stay empty).
//
// RATCHETED AT ZERO: the old empty-baseline machinery (`KNOWN_CYCLES`,
// `MECHANISM_EDGE_ALLOWLIST` and their shrink-only comparison arms) is retired — both baselines
// burned down to empty and enforcement is identical at zero. Any future cycle or mechanism→feature
// edge exception is a REVIEWED RE-INTRODUCTION of the ratchet machinery with operator
// confirmation (strictly more friction than a baseline entry), never a silent allowlist. Rule E's
// non-empty shrink-only census (`LEGACY_REGISTRANTS`) keeps its stale arm untouched.
//
// The rules:
//   A. No production import cycles — zero, with no baseline.
...
//      seam stays importable). Zero edges, with no allowlist. Module-level bans on the
//      resolved-edge map (exact module paths as `startsWith`
...
//      re-exports, and a future legitimate need is a reviewed guard edit,
...
import {
  ...
  checkDirCensus,
  checkDirection,
  checkRegistrationConfinement,
  checkTokenCensus,
  extractSpecifiers,
  findCycles,
  resolveRelative,
} from "./testing/importGraph.ts";
 ...
 * rule helper: the underlying edge-map/extractor machinery is already control-proven), shared by
 * the production assertion and its mutation control so the control
 ...
 * the envelope, and the graph machinery's controls ride there), and this guard file itself (the
 * one documented self-exemption: it necessarily names the tokens it polices).
 */
...
test("Rule A: no production import cycles", () => {
  const cycles = findCycles(scan().edges);
  assert.deepEqual(
    cycles,
    [],
    `production import cycle(s):\n${formatCycles(cycles)}\n` +
      "Break the cycle (invert or delete the closing edge) — enforcement is ratcheted at zero: " +
      "an exception is a reviewed re-introduction of the ratchet machinery with operator " +
      "confirmation, never a silent baseline.",
  );
});

test("Rule B: stable mechanisms never import features", () => {
  const violations = checkDirection(scan().edges, MECHANISM_HOMES, FEATURE_HOMES);
  assert.deepEqual(
    ...
    "mechanism→feature import(s): mechanisms take dependencies as parameters; " +
      ...
      "in from the feature side) — enforcement is ratcheted at zero: an exception is a reviewed " +
      "re-introduction of the ratchet machinery with operator confirmation, never an allowlist.",
  );
});
...
test("Rule G: report-wave transport confinement (interior ban; supplier floor; token census)", () => {
  ...
  assert.deepEqual(
    checkTokenCensus(censusFiles, readProductionFile, TRANSPORT_TOKEN),
    [],
...
test("Rule H: storage-free feature homes never import the storage interior", () => {
  const { files, edges } = scan();
  const violations = checkDirection(edges, STORAGE_FREE_HOMES, STORAGE_INTERIOR);
  assert.deepEqual(
    violations.map((edge) => `${edge.from} → ${edge.to}`),
    [],
    "storage-interior import(s) from a storage-free feature home: session identity, workflow " +
      ...
      "interior. Enforcement is ratcheted at zero; a legitimate need is a reviewed guard edit.",
  );
...
// ---------------------------------------------------------------------------------------------
// Non-vacuity controls (policy-coupled: real constants, real-scan mutations). The pure-machinery
// controls (specifier extraction, cycle fixtures, extensionless-phantom resolution, census
// fixtures) live with the machinery in extension/testing/importGraph.test.ts; the numbering
// below keeps its historical gaps — the moved controls kept their fixtures, and the old
// baseline-ratchet controls retired with the ratchet machinery.
// ---------------------------------------------------------------------------------------------
...
test("control 1: corpus + edge-map floors and known anchors", () => {
  ...
  assert.ok(
    (edges.get("pi/v1/providers/plannotatorHandoff.ts") ?? []).includes("pi/v1/reviewOutcome.ts"),
    "edge map missed the live type-only edge pi/v1/providers/plannotatorHandoff.ts → pi/v1/reviewOutcome.ts",
  );
});
...
test("control 4: the resurrection mutation — re-adding bindings→config is caught", () => {
  ...
  const cycles = findCycles(mutated);
  assert.ok(
    cycles.some(
      (cycle) => cycle.includes("substrate/bindings.ts") && cycle.includes("substrate/config.ts"),
    ),
    "the resurrected config↔bindings cycle was NOT reported",
  );
});
...
test("control 13: Rule G mutation fixtures (interior edge; supplier floor; token word bounds)", () => {
  ...
  const violations = checkTokenCensus(
