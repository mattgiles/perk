...
// published `code-review` action, the outcome mapping (handled / unavailable), and the
// presence helper. Fully offline — the fake plannotator is a test listener on an event bus that
// calls `respond(...)`. See prReviewLocal.ts.
...
interface CodeReviewEnvelope {
  requestId: string;
  action: string;
  payload: { prUrl: string; cwd: string };
