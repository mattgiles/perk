...
// records the reviewed bytes and returns a canned outcome, and a seeded browser-review scaffold
// (a git repo with one `origin` remote — the destination fence needs a verifiable checkout — plus
// the subject's draft artifact written through the branch session) returning a fresh slot.
// Outside the production corpus, the guard scans, and the npm tarball (the `testing/` home).
