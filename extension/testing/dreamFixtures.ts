...
// its bound manifest digest). Both objective boundary suites (objectiveDraft.test.ts,
// objectiveSave.test.ts) consume this one encoding of the persisted format instead of each
// maintaining a copy; the resolver suite (objectiveDreamReport.test.ts) keeps its own richer
// 2-lane fixture because it exercises proposal/stance shapes and per-arm corruption this
// minimal fixture deliberately lacks. Test-only — never imported by production modules.
...
/** The minimal 1-lane / 1-doc raw producer manifest. */
export function dreamRawManifest(): Record<string, unknown> {
  return {
    schema_version: "1",
    commit_sha: "abc123",
...
export function dreamManifestBytes(): string {
  return `${JSON.stringify(dreamRawManifest(), null, 2)}\n`;
}
...
/**
 * Plant the run-scoped dream files: the manifest, and (unless `finalized: false`) the
 * FINALIZED bundle binding the manifest bytes' digest. Returns the `dream_bundle_digest`
 * marker value for the planted state (`""` when not finalized — the caller decides where the
 * marker entry lives: a fake branch array or a planted session). Throws on a broken fixture
 * (test-only code).
 */
export function plantDreamFiles(
  ...
  const manifestBytes = dreamManifestBytes();
