...
test("reviewer definition and managed mirror pin assessment and exact context acceptance", () => {
  ...
  for (const rule of [
    ...
    /explicit `null` \(or a file with blank text\) is valid optional\s+evidence for non-plan-fidelity lanes/,
    /A\s+referenced file that\s+cannot be read \(missing, unreadable\) blocks the lane/,
    /a long line is never by itself a reason to block/,
    /sed -n '<N>p' <path> \| tail -c \+<offset> \| head -c 51200/,
    /until a slice comes back empty/,
    /No other listed field is optional/,
    /Do\s+not compare them to the current local branch/,
    /infer a different PR, add head-SHA binding, or\s+fetch again/,
