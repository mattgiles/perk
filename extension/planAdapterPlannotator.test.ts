...
// `plan_review` bridge tool's full path matrix — soft skips (not selected / headless), the bounded
// handshake (timeout / unavailable), the human decision (approved / denied + feedback), and the
// turn-abort path. Fully offline: the fake plannotator is a test listener on an event bus that
// calls `respond(...)` and emits `plannotator:review-result`. See planAdapterPlannotator.ts.
...
test("bridge: approved decision -> completed outcome; result says APPROVED + /plan-save", async () => {
  const bus = fakeBus();
  bus.on("plannotator:request", (data) => {
  ...
  assert.match(text, /ship it; also note the edge case/);
