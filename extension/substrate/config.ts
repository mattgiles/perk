...
/**
 * The ONE committed-only, main-checkout-anchored reader of the `[issues]` table's destination
 * keys — the raw `backend`/`team` strings that select WHERE an approval saves.
 *
 ...
 * stale branch / missing `.perk/`) must never flip a Linear repo's reading to GitHub.
 * Absence is a VALUE: a missing key/table/file reads as `null` (a malformed file reads as
 * absent too — loudly, via `readTomlFile`'s stderr warning); non-string values read as `null`;
 * any throw reads both as `null`. Nothing else in the config is consulted, so unrelated
 * `.perk/*.toml` edits never change the reading (contracts.md §8.23).
 */
export function issueDestination(cwd: string): { backend: string | null; team: string | null } {
  try {
    const issues = readTomlFile(configFile(mainCheckoutRoot(cwd))).tables.issues;
    return {
      backend: typeof issues?.backend === "string" ? issues.backend : null,
      team: typeof issues?.team === "string" ? issues.team : null,
    };
  } catch {
...
/**
 * The fail-safe TS mirror of the issue-backend selection, on top of `issueDestination`'s
 * committed-only main-checkout read. Python (`perk/backends/resolve.py::resolve_issue_backend_id`)
 * is the AUTHORITATIVE validator and **raises** on unknown values; this mirror is fail-safe
 * (absence/unknown/any error → `"github"`) because the TS plane only renders prompts — it never
 * writes canonical issues.
 */
export function resolveIssueBackendId(cwd: string): IssueBackendId {
  const backend = issueDestination(cwd).backend;
  return backend === "github" || backend === "linear" ? backend : GITHUB_ISSUE_BACKEND_ID;
}
