...
// walking. The clone has no `node_modules` (pi skips `npm install` on an existing pinned ref), so a
// bare `import { parse } from "yaml"` is unresolvable and the whole extension fails to load. The
// fix is to make the extension self-contained: this reader replaces the lone non-host runtime
// import (`yaml`) with Node builtins only — NO bare npm imports.
//
