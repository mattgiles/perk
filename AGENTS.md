...
- **Regression-testing discipline.** Regression coverage lives in the two framework suites —
  **`pytest` (preferred) and `node:test`** — run by `just test` and gated by `just ci` (which must
  stay green). In perk sessions, run these through the `run_ci` tool / `/ci` — the `[[ci.checks]]`
  rows mirror `just ci`'s verification targets (lint/typecheck/test/changelog-check; `setup` is env
  prep owned by the `[worktree] setup` hook) — rather than bare `just ci` in bash, and treat a
  green run-all report as definitive: no follow-up re-verification. Grow a Python test harness when it widens what the suite checks (e.g.
