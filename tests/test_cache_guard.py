...
ALLOWED = frozenset({"state/cache.py"})
...
class TestCachePathGuard:
    def test_no_production_module_builds_scratch_paths_directly(self) -> None:
        """Source scan: outside perk/state/cache.py (the seam), no module under perk/ may construct
        ...
        assert not offenders, (
            "manual scratch/runs path construction outside perk/state/cache.py — go through the "
    ...
    def test_pattern_matches_the_seam_itself(self) -> None:
        ...
        source = (_perk_dir() / "state" / "cache.py").read_text(encoding="utf-8")
