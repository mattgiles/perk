...
def test_launch_injects_no_fff_mode_and_forwards_the_operator_value():
    """perk injects no `PI_FFF_MODE` (pi-fff's own precedence decides its search mode; the
    0.67.x pi-subagents host-tool hazard the injected default guarded is gone), and an
    operator-set value passes through verbatim by merge order."""
    absent = _build_exec_env(
        run_id="01TEST",
        environ={},
        fallback_linear_api_key=None,
        pi_agent_dir=None,
    )
    assert "PI_FFF_MODE" not in absent
    forwarded = _build_exec_env(
        run_id="01TEST",
        environ={"PI_FFF_MODE": "override"},
        fallback_linear_api_key=None,
        pi_agent_dir=None,
    )
    assert forwarded["PI_FFF_MODE"] == "override"
