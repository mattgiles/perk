...
def test_real_launch_defaults_forward_no_override_and_no_plan_id(monkeypatch, tmp_path):
    # The two positioning fields default None: the registry stage launches verbatim and the
    # bare-id passthrough stays empty (every non-positioning door is byte-unaffected).
    captured: dict = {}
    monkeypatch.setattr(launch, "launch_stage", lambda **k: captured.update(k))
    result = _invoke(_toy_door(lambda r, c, s: _spec()), ["--json"], tmp_path)
    assert result.exit_code == 0
    assert captured["stage"].id == "plan"
    assert captured["stage"].worktree == "none"
