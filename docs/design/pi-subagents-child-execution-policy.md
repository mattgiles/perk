...
| Completion guard | Definition `completionGuard: false` (the engine's completion **mutation** guard is off: a report-only lane whose task text reads as implementation intent — e.g. a reviewed draft saying "… must change …" — completes on its validated `structured_output` report without editing). Separate from acceptance (`WAVE_ACCEPTANCE` suppresses the acceptance contract; this suppresses the missing-edit classification). The exemption does NOT bypass the required report contract: a missing or schema-invalid report still fails the lane, and a failed lane stays uncovered downstream. Non-mutation remains the job of Perk's restrictions + the categorical rubric prohibitions, never of this field (inspection `bash` stays available). `extension/waves/reportOnlyCompletionCompat.test.ts` proves it through the real supplier | Definition field absent (engine default: guard enabled) — the writer IS expected to mutate |
