...
- Every routed page appears in the sidebar or is explicitly recorded here as a
  non-navigation page. **As of this blueprint there are no non-navigation pages: all 70
  routed pages appear in the sidebar.**
...
1. **Core workflow** — drive-the-full-spine, resume-a-plan, address-review-feedback,
   review-a-foreign-pr, replan-an-open-plan, adopt-an-existing-issue, capture-a-gist,
...
2. **Objectives & learnings** — author-a-roadmap, replan-an-objective, advance-or-skip-nodes,
   reconcile-an-objective, check-an-objective-for-drift, run-the-learn-docs-factory,
...
| `providers-and-backends.md#issue-backend--linear-reference` | `providers-and-backends/issue-backends.md#linear` — the Linear reference lands on the child under a `## Linear` heading; node 4.4 updates the inbound links (`how-to/switch-to-linear.md`, `reference/configuration.md`) and records the final mapping here per the migration policy. |

#### Part 2 — family-assignment rule for all remaining anchors

Every H3 entry (a command, a config table, or a tool) not listed above migrates to the child
that owns its family **per the §4 family lists** — deterministic, since §4 enumerates which
command groups, config tables, and tool families belong to which child — **keeping its slug
on the child page**. Examples: `cli.md#perk-plan-watch-plan` → `cli/plan.md#perk-plan-watch-plan`;
...
in this table — executed by nodes 4.1–4.4, each re-running the Part 1 sweep for its page
before it lands.
...
repos, the masked `PERK_GH_PAT`, the runner-enable variable, and Actions capacity; captures
...
PAT/variable are provisioned and re-proven by the preflight itself.
...
| Runner-enable variable | `confirmed` | `runner-enabled: info — remote runner enabled (PERK_ENABLED unset → default-on)` (this repo) |
