...
- The **explicit initial sidebar** — navigation is Starlight's autogenerated default until
  then (no `sidebar` key in `astro.config.mjs`); the `sidebar.order` values reproduce the
  blueprint §3 section order in the interim.
- **Site** validation/build/search gating: no `@astrojs/check` / `docs-check`, no site
  type-checking in `just typecheck`, no docs validation/build/search in `just test`, and no
  docs `[[ci.checks]]` row. (The live-corpus **pytest** metadata/accounting guard
  `tests/test_user_docs_metadata.py` is already active in `just test`; site *lint* is covered —
  the root `lint` script runs `biome check extension docs/site` — and the bridge plugins' own
  `node:test` unit tests run in `just test`.)
