...
resolution — and plan draft review via `/plan-review-browser`; the `draft-reviewer` def's
objective arm awaits its door) to subagents via the `pi-subagents` package. perk's agent defs — the `PERK_AGENTS`
