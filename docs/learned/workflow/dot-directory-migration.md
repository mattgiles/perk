...
A multi-node arc (objective #878) moves perk's owned dot-directory state out of the shared `.pi/`
namespace into a dedicated `.perk/` root. This doc captures the durable cross-cutting mechanics —
...
Two per-plane leaf seams — `perk/substrate/paths.py` + `extension/substrate/paths.ts` — own
...
`rmdir` the legacy root only when it is empty. Legacy paths are written as **frozen flat-string
literals** (`Path(".pi/skills")`, `root / ".pi/workflow"`) to stay exempt from the operator-adjacency
paths guard — they are deliberately not routed through the seam because the seam now points at the
new home.
...
The block is generated by `perk/convergence/init/blocks.py::_agents_inner()`. After editing the
...
narrative cites the merge PR). As of these learnings:

- **1.1** (path seam), **3.1** (workflow cache), **4.1** (repo-skills), **5.1** (docs) — LANDED;
- **2.1** (config `.pi/perk.toml`→`.perk/config.toml`) — **CLOSED UNMERGED, still in-progress**;
- **6.1** (self-migration dogfood of the config + workflow/cache ignore-state) — pending (a slice
  was pulled forward to land 3.1).

**Do not document the config move as complete.**
...
- `docs/learned/workflow/config-tables.md` — the config-file readers the 2.1 move touches
