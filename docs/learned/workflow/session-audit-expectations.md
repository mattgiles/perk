...
(`extension/learning/audit.ts::judgeAuditBundle` behind the `extension/pi/v1/learning/audit.ts`
adapter) writes `verdicts.json`,
...
- **The verdict writer has a post-dispatch I/O failure arm.** A `verdicts.json` write failure
  *after* the wave launched returns `io_error` with in-memory lane records and **no guaranteed
  ...
  `extension/learning/audit.ts` (adapter: `extension/pi/v1/learning/audit.ts`); the fold side in
...
Residual (reconciled after the first live corpus pass — `docs/design/archive/session-audit-dogfood.md`):
