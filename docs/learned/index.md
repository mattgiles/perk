...
| workflow | [cold-door-client.md](workflow/cold-door-client.md) | Adding a warm door that shells to a `--json` cold door (rollout complete — the substrate is mandatory), writing a cold-door envelope decode (strict vs advisory vs fully-lenient), consuming a fail-arm payload, the fixture-sweep rule when strictening a decode, the legacy-label byte-compat lever, or door-test assertion churn after a migration |
| workflow | [github-gateway.md](workflow/github-gateway.md) | Touching `perk/github.py`, consolidating repeated subprocess/parse idioms (the parse-helper family + boundary rules), debugging a phantom-`None` GitHub lookup (the not-found fold), or adding a REST/GraphQL call |
...
| pi | [extension-api.md](pi/extension-api.md) | Live system-prompt inputs in extensions, registerTool execute results details requirement, custom tool read-only gating traps, tsconfig strict-mode index access in tests, command vs event handlers, injected-message persistence, testing `pi.events`-bridge logic / flag-shortcut non-registration from the harness, asserting a `pi.sendUserMessage` injection offline, or the `headfulUIContext` select/input gap |
| pi | [extension-seams.md](pi/extension-seams.md) | Collapsing a repeated context-dependent extension idiom into one tested seam (the `report()`/`branchOf`/`EntrySink` minimal-structural-interface recipe), the strict workflow-state append + read-back tiering, the seam-owned-prefix trap, or the P1/P2/P3 site triage for what a single-message seam can absorb |
| pi | [tool-param-decode.md](pi/tool-param-decode.md) | Decoding registered-tool params (the tri-state `toolParams.ts` seam), adding a tool handler, choosing strict vs lenient decode semantics at a boundary, or testing decode-before-side-effect ordering |
...
| toolchain | [ruff.md](toolchain/ruff.md) | Ruff check vs format split, silent-failure commit-reformat traps, RUF100 unused noqa rules, template string E501 multiline limits, SIM105 exception suppressions, or preview-rule enablement dragging cross-bucket fixes forward |
