...
(with the `[ci]` checks table **commented out** by default). It is idempotent — re-running it
...
scratch repo configured no checks (the `[ci]` table is commented out), so perk reports there
