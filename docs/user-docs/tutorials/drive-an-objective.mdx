...
is verified. That is the only prerequisite — this lesson starts from a fresh scratch repo.
...
perk objective show <N> --full   # show the objective's body, including its reconcilable prose
```

On GitHub the objective's prose lives in the objective-body comment that the objective's metadata
references (not in the issue body, so plain `gh issue view` misses it) — `--full` reads it for you
and prints it as an `<untrusted_objective_body>` block. When reconciliation changed stale prose,
this view shows the updated body. When nothing drifted, the reconcile turn instead reports its
