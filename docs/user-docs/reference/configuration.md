...
| `session-auditor` | string (model id) | _(agent frontmatter default)_ | **[Dev-only](../../developers/session-audit.md)** — model for the session-auditor agent (perk's own repo's session-audit judgment wave, spawned inside a `perk-dev audit judge` session; the agent def is repo-local, not delivered by `perk init`, so this key is dormant in consumer repos). |
