...
executor (warm `/ci` + the `run_ci` tool) and run automatically at `/ready`. Declared order is
preserved.
...
| `review` | string | `hunk` | The code-review surface the `/review` door drives (selectable: `plannotator-review` — refused until that arm lands). |
...
| `guest-reviewer` | string (model id) | _(agent frontmatter default)_ | Model for the guest-reviewer agent (foreign-PR review; spawned by `/review`). |
