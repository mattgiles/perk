...
Author an **advisory refinement** of a future roadmap node — a dated, target-bound note that
sharpens a `pending`/`blocked` node with no plan yet — in a fresh **read-only** `objective-refine`
session (the refinement is saved as the node's single marked comment; nothing else changes — no
