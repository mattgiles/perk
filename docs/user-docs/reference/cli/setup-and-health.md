...
project entry with `"autoload": false` does not — pi keeps that delta pair on purpose). perk
...
but the duplicate still costs a listener and the warning, so the check names both absolute paths
and the remediation: remove the user-scope entry (`pi remove npm:pi-subagents` when that file is
...
path when the user settings are not valid JSON; a warn deferring to `settings-wiring` when the
project settings are malformed; `ok` ("configured in project scope only") otherwise.
