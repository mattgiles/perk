...
2. **Act on the reported outcome.** The supervisor uses these current action boundaries:
   ...
   | `ready_for_review` | Implementation produced a draft pull request; review it on the draft first. `/ready` is the human boundary — the review gate for an incremental plan, the post-review handoff stamp for a stacked layer. |
