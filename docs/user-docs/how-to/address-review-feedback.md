...
description: "Classify reviewer feedback on a plan's PR, fix the actionable items, and resolve the threads — then hand the PR back to review and land (or, on a stacked layer, record the handoff)."
sidebar:
  order: 2030
...
then hand the PR back to review and land (or, on a stacked layer, record the handoff). Use this
when a reviewer has left comments on the draft/ready PR.
...
4. **Review and let perk resolve.** Confirm the classification, let perk apply the fixes and resolve
   the addressed threads. `finalize_address`'s result ends with a `Hand-off` line naming the right
   post-approval gesture for this plan's delivery kind (`/land` on an incremental plan, `/ready` on
   a stacked layer) — the session relays it in its closing summary.
5. **Re-ready and land (incremental).** Once the feedback is addressed and committed, run warm
...
> fix. The hand-off differs: once the layer is approved, `/ready` records the handoff stamp — never
> `/land` (it refuses with `stacked_plan`); the train lands whole via `/objective-land`. See
> [How to review a stacked PR train](./review-a-stacked-train.md).
