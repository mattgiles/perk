...
1. **List the runs.** Run
   ...
   **read-only** — it mutates nothing — and its live GitHub overlay is best-effort and fail-soft. Use
   `--no-refresh` to skip the overlay, `--limit` to bound the rows, or `--json` for machine-readable
   output.
