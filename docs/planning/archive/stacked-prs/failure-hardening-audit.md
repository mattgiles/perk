...
> [`stacked-delivery-dogfood.md`](../../../design/archive/stacked-delivery-dogfood.md) **PASSED**. Real
...
real residue), then run the public rerun/recovery surface. Operation-conclusion evidence now runs
through `Delivery.recover(RecoverRequest(...))` over the three aggregate authorities; direct
recovery dependency-bag construction is not proof of the public path. A fail-once raise is
permitted
...
| 12 | External prefix merges | `tests/test_delivery_recover.py::test_terminal_probe_prefix_classifies_external_prefix`, `::test_accept_prefix_journals_the_breach_and_finalizes_the_prefix_only`, `::test_accept_prefix_membership_change_after_confirmation_is_accept_blocked`, `::test_accept_prefix_on_a_non_external_target_is_accept_blocked`, `::test_accept_prefix_declined_journals_nothing`, `::test_transfer_rejects_accept_prefix_before_classification_or_sweep` (all-before + all-after), `::test_drifted_remainder_head_is_mixed_not_external_prefix`, `::test_non_prefix_drift_arms_are_mixed_and_journal_nothing` | The contiguous-prefix acceptance is explicit (`--accept-prefix` + confirm), LAND-only, and journaled as a degraded-atomicity breach; TRANSFER rejects the action before observation/effects, and anything non-prefix is mixed, fail closed. |
| 13 | Post-merge bookkeeping failures | `tests/test_delivery_landing.py::test_completed_append_failure_degrades_to_merged_with_note`, `::test_completed_append_store_failure_degrades_to_merged_with_note`, `::test_finalize_failure_notes_and_remaining_layers_still_finalize`, `::test_consumed_learn_read_failure_finalizes_without_it`, `::test_aggregate_close_fail_open`; convergence: `tests/test_delivery_recover.py::test_roll_forward_finalize_failure_is_isolated_and_loud`, `::test_roll_forward_completed_append_failure_degrades_and_the_rerun_converges`, `::test_convergence_refinalizes_every_covered_corroborated_layer`, `::test_transfer_result_metadata_failure_precedes_and_suppresses_sweep` | Bookkeeping failures degrade loudly and converge on a later recover pass; the merge itself is never re-attempted. Recovery reads fallible result metadata before the final orphan sweep, so a typed failure cannot leave cleanup preceding a failed operation. The L7 close-then-evidence window was the one real hole — fixed by the earlier hardening pass (see the LAND matrix). |
...
| Non-journaled conversion windows | `test_non_journaled_conversion_interrupted_mid_ownership_converges_by_construction`, `test_non_journaled_conversion_interrupted_after_the_finalize_stamp_re_finalizes`, `test_interrupted_conversion_rerun_converges_the_clear_writes` — hermetic façade-equivalence evidence only. This does **not** discharge the real-Linear post-MOVE/pre-ownership window: without a journaled intent, partial remote state cannot safely identify the preflighted request on a later run. |
...
| Cross-machine conclusion | `tests/test_delivery_cross_machine.py::test_transfer_manifest_rolls_forward_on_fresh_seams` (fresh Delivery with aggregate authorities; the façade binds private `TransferSeams`; ownership/finalize/completion each exactly once) |
