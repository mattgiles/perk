...
| 4 | Atomic-push refusal (unsupported capability) | **New:** `tests/test_git.py::test_probe_atomic_push_refuses_a_capability_suppressed_transport`, `::test_push_atomic_with_leases_capability_suppressed_transport_moves_no_ref`; `tests/test_delivery_capability.py::test_probe_atomic_push_urls_real_probe_against_a_non_atomic_transport_fails_the_check`. Operational gate: `tests/test_delivery_sync.py::test_atomic_push_unsupported` | The real refusal transport is `receive.advertiseAtomic=false` — the server stops advertising the capability and the *client* refuses (`does not support --atomic`), which is the named class. A `pre-receive` hook rejection would prove policy refusal, a different class — never recorded against this cell. Production `capability.py` untouched (it already converts any `GitError` into a failed `atomic-push` check). |
