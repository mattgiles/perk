...
| [design/stacked-delivery-dogfood.md](./design/stacked-delivery-dogfood.md) | The **stacked-delivery dogfood gate** validation record (Objective #1431 node 6.2): the repeatable live full-lifecycle protocol — warm stacked authoring → build-readiness planning → second-clone layer implementation → lower-layer feedback + suffix cascade → ready → the atomic merge-async landing, deliberately interrupted after the journal's `accepted` event and concluded from the second clone by `stack recover` → reconcile → census — driven on a real 3-layer docs train whose merged layers ARE the node's documentation deliverables; dated 2026-08-13 evidence + defect log — overall **`PASS`** (all planned arms, including interruption→`all_after` conclusion; follow-ups #1709/#1710 for two non-blocking defects). Superseded-in-part 2026-08-22: its pre-stamp ready evidence yields to the stamped-lifecycle record below. |
| [design/stacked-ready-handoff-dogfood.md](./design/stacked-ready-handoff-dogfood.md) | The **stamped stacked-lifecycle dogfood gate** validation record (Objective #1951 node 4.1): the source-verified five-family coherence-audit matrix (status / next-action / dry-run / warm-door / cold-door — runtime surfaces agree; every finding doc-side) plus the repeatable live protocol — publish → handoff stamp (warm `/ready` AND the flat `perk ready <PLAN>` wrapper) → the typed `node_not_handoff_ready` planning gate → the ready-time reconcile continuation (bounded writes AND audited no-ops) → the suspend hold + idempotent re-stamp → self-rewrite AND cascade staleness → wrapper re-ready repair → the stamped train's atomic landing + near-no-op post-land pass — driven on a real 2-layer docs train (#1980) whose merged layers ARE the node's teaching deliverables; dated 2026-08-22 evidence — overall **`PASS`** (every arm, zero defects). |
| [design/sync-conflict-resolution-dogfood.md](./design/sync-conflict-resolution-dogfood.md) | The **warm sync-conflict drive dogfood** validation record (Objective #2071 node 3.1): the repeatable happy-path live protocol for the automated conflict-resolution loop — a human-approved mutating base cascade → the corroborated `rebase_conflict` stop (freshness token + warm-route hint) → the auto-dispatched retained-continuation resolver → a `completed` outcome with zero resolver-driven publication → the explicit human `continue` → a clean train at the new anchor — driven on objective #2040's genuinely stalled 10-layer docs train; every failure arm offline-pinned to its hermetic suite; dated 2026-08-23 evidence — overall **`PASS`** (all five criteria observed live on the first attempt, zero defects, five named non-defect observations). |
...
| [design/learn-dream-dogfood.md](./design/learn-dream-dogfood.md) | The **learn-dream dogfood** validation record (Objective #1892 node 5.2): the repeatable single-leg live acceptance protocol for `perk learn dream` (origin preflight → dirty-refusal probe → the full-corpus two-level analyst/reducer wave session → the overlap-refusal probe when applicable → census), the bounded attempt/outcome state machine with the three honest terminals, the per-terminal acceptance-criteria evidence map, and the dated 2026-08-20 evidence — overall **`PASS`** (attempt 1, `SAVED_OBJECTIVE` — curation objective #1926 retained as real backlog, full 14+3-lane wave coverage, both live probes fired, two environment/deviation D-rows, no dream defect, successor 5.5 for the subagent-guidance re-verify; the Linear live proof dropped by owner decision — offline parity pins only, not-live-proven). |
...
| [design/docs-site-modernization-walkthrough-evidence.md](./design/docs-site-modernization-walkthrough-evidence.md) | The **docs-site modernization hardening record** (Objective #1898 node 3.1): the dated 91-shot rendered sweep, diagram no-JS/print/keyboard/reduced-motion passes, live §9/§11/§12 contrast recomputation, §1–§12 blueprint↔artifact agreement table, explicit screen-reader waiver and obsolete dreaming-marker disposition, and defect/rerun log. |
