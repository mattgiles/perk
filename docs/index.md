...
| [design/learn-dream-dogfood.md](./design/learn-dream-dogfood.md) | The **learn-dream dogfood** validation record (Objective #1892 node 5.2): the repeatable single-leg live acceptance protocol for `perk learn dream` (origin preflight → dirty-refusal probe → the full-corpus two-level analyst/reducer wave session → the overlap-refusal probe when applicable → census), the bounded attempt/outcome state machine with the three honest terminals, the per-terminal acceptance-criteria evidence map, and the dated evidence — overall **(pending — gate not yet run)**. |
