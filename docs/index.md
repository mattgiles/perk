...
| [design/pi-subagents-child-execution-policy.md](./design/pi-subagents-child-execution-policy.md) | The **native child execution and advisory scratch-identity decision** for Objective #2209 node 3.1: closed eleven-role census, background reports, foreground writers with actual cwd, independent warm-parent read-only restrictions, exact advisory-identity fallback rules, and named implementation/test duties for nodes 3.2/3.3. Its approval pointer records the human-review status; the linked archive preserves native measurements and teardown. |
