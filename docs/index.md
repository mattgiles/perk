...
| [releasing.md](./releasing.md) | The **maintainer release runbook + policy**: the **version graph** naming every version-bearing surface and its owner (SSOT `pyproject.toml` `[project] version` → `perk.__version__` → `package.json`), semver/pre-1.0 policy, the **two-phase changelog convention** (`<!-- As of <hash> -->` marker + parenthesized short-hash tokens, stripped at release), the coordinated dual-plane (PyPI + npm) runbook, and the `validate-release-versions` tag gate. |
