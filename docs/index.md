...
| [design/pi-subagents-child-execution-policy.md](./design/pi-subagents-child-execution-policy.md) | The **native child execution policy**: the two booleans (runner bit; the constant report restriction packet), the monotone read-only floor, caller-checkout placement, the dropped mechanisms and what the policy does not claim; the archive holds the historical characterization. |
