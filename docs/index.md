...
| [releasing.md](./releasing.md) | The **maintainer release runbook + policy**: the version SSOT chain (`pyproject.toml` `[project] version` → `perk.__version__` → `package.json`), semver/pre-1.0 policy, CHANGELOG discipline, the coordinated dual-plane (PyPI + npm) runbook, and the `validate-release-versions` tag gate. |
| [release-checklist.md](./release-checklist.md) | The hand-held first-release checklist: account setup, package-name checks, GitHub environments, PyPI trusted publishers, npm `NPM_TOKEN`, TestPyPI rehearsal, tag publish, and post-release verification. |
