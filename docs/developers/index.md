...
| [Profiling perk's startup](./profiling-startup.md) | How-to | you want to measure perk's cold startup repeatably (`perk-dev profile-startup`), prepare a subject checkout, read a run directory, use the `PERK_PROFILE_HANDOFF` seam / cProfile / importtime / the Node module census, compare two revisions (a pinned baseline worktree vs a candidate) or produce a baseline / comparison record |
