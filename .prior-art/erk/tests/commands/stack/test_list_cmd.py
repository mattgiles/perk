...
def test_list_graphite_not_enabled() -> None:
    """Test stack list command requires Graphite to be enabled."""
    runner = CliRunner()
    with erk_inmem_env(runner) as env:
        ...
        result = runner.invoke(cli, ["stack", "list"], obj=test_ctx, catch_exceptions=False)
...
def test_list_graphite_not_installed() -> None:
    """Test stack list command shows appropriate error when Graphite CLI is not installed."""
    runner = CliRunner()
    with erk_inmem_env(runner) as env:
        repo_dir = env.setup_repo_structure()
