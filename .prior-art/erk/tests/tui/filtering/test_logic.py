...
def test_filter_by_title_substring() -> None:
    ...
    plans = [
        make_pr_row(1, "Add user authentication"),
        make_pr_row(2, "Fix login bug"),
        make_pr_row(3, "Refactor database"),
    ]
...
def test_filter_by_pr_number() -> None:
    """Filters plans by PR number (pr_number is the issue/PR number)."""
    plans = [
        make_pr_row(100, "Plan A"),
        make_pr_row(200, "Plan B"),
        make_pr_row(300, "Plan C"),
    ]
...
def test_partial_issue_number_match() -> None:
    ...
    plans = [
        make_pr_row(123, "Plan A"),
        make_pr_row(456, "Plan B"),
        make_pr_row(1234, "Plan C"),
    ]
    # "12" matches both 123 and 1234
    result = filter_plans(plans, "12")
    assert len(result) == 2
    assert result[0].pr_number == 123
    assert result[1].pr_number == 1234


def test_filter_by_author() -> None:
    """Filters plans by author name."""
    plans = [
        make_pr_row(1, "Plan A", author="alice"),
        make_pr_row(2, "Plan B", author="bob"),
        make_pr_row(3, "Plan C", author="alice-smith"),
    ]
...
def test_multiple_matches_preserved_order() -> None:
    ...
    plans = [
        make_pr_row(1, "First feature"),
        make_pr_row(2, "Second feature"),
        make_pr_row(3, "Third feature"),
    ]
