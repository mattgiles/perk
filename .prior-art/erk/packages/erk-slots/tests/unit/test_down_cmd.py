...
def test_down_worktree_branch_mismatch() -> None:
    """Test down command when worktree exists at expected path but has different branch checked out.

    This simulates the scenario where a user ran `git checkout other-branch` inside
    a worktree that was originally created for feature-1. The worktree directory
    still exists at the feature-1 path, but has a different branch checked out.
    Running `erk down` should navigate to that worktree and include a `git checkout`
    ...
    with erk_inmem_env(runner) as env:
        ...
        # Worktree at feature-1 path has "other-branch" checked out (mismatch)
        feature_1_path = repo_dir / "worktrees" / "feature-1"
        worktrees_list = [
            WorktreeInfo(path=env.cwd, branch="main", is_root=True),
            WorktreeInfo(path=feature_1_path, branch="other-branch", is_root=False),
            WorktreeInfo(
                path=repo_dir / "worktrees" / "feature-2", branch="feature-2", is_root=False
            ),
        ]
