...
def extract_pr_metadata(tool_result_content: str) -> dict[str, str | int | None]:
    ...
    Returns:
        Dict with pr_url, pr_number, and pr_title (all may be None)

    Example:
        >>> content = '{"success": true, "pr_url": "https://...", "pr_number": 123, '
        >>> content += '"pr_title": "Fix bug"}'
        >>> extract_pr_metadata(content)
        {'pr_url': 'https://...', 'pr_number': 123, 'pr_title': 'Fix bug'}
    """
    if not isinstance(tool_result_content, str):
        return {"pr_url": None, "pr_number": None, "pr_title": None}
...
def extract_pr_metadata_from_text(text: str) -> dict[str, str | int | None]:
    ...
    Returns:
        Dict with pr_url, pr_number, and pr_title (pr_title always None)
